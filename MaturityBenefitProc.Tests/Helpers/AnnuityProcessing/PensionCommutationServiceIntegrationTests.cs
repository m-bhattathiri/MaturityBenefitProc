...
{
    [TestClass]
    public class PensionCommutationServiceIntegrationTests
    {
        private PensionCommutationService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(PensionCommutationService));
            Assert.IsTrue(_service.GetType().Name == "PensionCommutationService");
        ...
        {
            var s1 = new PensionCommutationService();
            var s2 = new PensionCommutationService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new PensionCommutationService()).ToList();
        ...
        {
            var type = typeof(PensionCommutationService);
            Assert.IsNotNull(type);
            Assert.AreEqual("PensionCommutationService", type.Name);
        ...
        {
            var ns = typeof(PensionCommutationService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(PensionCommutationService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new PensionCommutationService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(PensionCommutationService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("PensionCommutationService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new PensionCommutationService();
