...
{
    [TestClass]
    public class OutstandingLoanRecoveryServiceIntegrationTests
    {
        private OutstandingLoanRecoveryService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(OutstandingLoanRecoveryService));
            Assert.IsTrue(_service.GetType().Name == "OutstandingLoanRecoveryService");
        ...
        {
            var s1 = new OutstandingLoanRecoveryService();
            var s2 = new OutstandingLoanRecoveryService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new OutstandingLoanRecoveryService()).ToList();
        ...
        {
            var type = typeof(OutstandingLoanRecoveryService);
            Assert.IsNotNull(type);
            Assert.AreEqual("OutstandingLoanRecoveryService", type.Name);
        ...
        {
            var ns = typeof(OutstandingLoanRecoveryService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(OutstandingLoanRecoveryService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new OutstandingLoanRecoveryService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(OutstandingLoanRecoveryService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("OutstandingLoanRecoveryService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new OutstandingLoanRecoveryService();
