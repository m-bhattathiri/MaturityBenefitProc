...
{
    [TestClass]
    public class TaxReportingServiceIntegrationTests
    {
        private TaxReportingService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(TaxReportingService));
            Assert.IsTrue(_service.GetType().Name == "TaxReportingService");
        ...
        {
            var s1 = new TaxReportingService();
            var s2 = new TaxReportingService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new TaxReportingService()).ToList();
        ...
        {
            var type = typeof(TaxReportingService);
            Assert.IsNotNull(type);
            Assert.AreEqual("TaxReportingService", type.Name);
        ...
        {
            var ns = typeof(TaxReportingService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(TaxReportingService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new TaxReportingService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(TaxReportingService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("TaxReportingService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new TaxReportingService();
