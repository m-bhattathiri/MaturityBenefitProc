...
{
    [TestClass]
    public class BankMandateServiceIntegrationTests
    {
        private BankMandateService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(BankMandateService));
            Assert.IsTrue(_service.GetType().Name == "BankMandateService");
        ...
        {
            var s1 = new BankMandateService();
            var s2 = new BankMandateService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new BankMandateService()).ToList();
        ...
        {
            var type = typeof(BankMandateService);
            Assert.IsNotNull(type);
            Assert.AreEqual("BankMandateService", type.Name);
        ...
        {
            var ns = typeof(BankMandateService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(BankMandateService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new BankMandateService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(BankMandateService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("BankMandateService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new BankMandateService();
