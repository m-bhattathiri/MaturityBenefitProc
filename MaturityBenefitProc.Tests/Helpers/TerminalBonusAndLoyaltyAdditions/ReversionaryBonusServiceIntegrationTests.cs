...
{
    [TestClass]
    public class ReversionaryBonusServiceIntegrationTests
    {
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(ReversionaryBonusService));
            Assert.IsTrue(_service.GetType().Name == "ReversionaryBonusService");
        ...
        {
            var s1 = new ReversionaryBonusService();
            var s2 = new ReversionaryBonusService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new ReversionaryBonusService()).ToList();
        ...
        {
            var type = typeof(ReversionaryBonusService);
            Assert.IsNotNull(type);
            Assert.AreEqual("ReversionaryBonusService", type.Name);
        ...
        {
            var ns = typeof(ReversionaryBonusService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(ReversionaryBonusService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new ReversionaryBonusService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(ReversionaryBonusService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("ReversionaryBonusService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new ReversionaryBonusService();
