...
{
    [TestClass]
    public class NreAccountValidationServiceIntegrationTests
    {
        private NreAccountValidationService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(NreAccountValidationService));
            Assert.IsTrue(_service.GetType().Name == "NreAccountValidationService");
        ...
        {
            var s1 = new NreAccountValidationService();
            var s2 = new NreAccountValidationService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new NreAccountValidationService()).ToList();
        ...
        {
            var type = typeof(NreAccountValidationService);
            Assert.IsNotNull(type);
            Assert.AreEqual("NreAccountValidationService", type.Name);
        ...
        {
            var ns = typeof(NreAccountValidationService).Namespace;
            Assert.IsNotNull(ns);
            Assert.IsTrue(ns.Contains("MaturityBenefitProc"));
            Assert.IsTrue(ns.Contains("International_NRI_Processing"));
        ...
        {
            var interfaces = typeof(NreAccountValidationService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new NreAccountValidationService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(NreAccountValidationService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("NreAccountValidationService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new NreAccountValidationService();
