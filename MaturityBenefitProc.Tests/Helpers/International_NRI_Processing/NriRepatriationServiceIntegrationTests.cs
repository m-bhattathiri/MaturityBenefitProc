...
{
    [TestClass]
    public class NriRepatriationServiceIntegrationTests
    {
        private NriRepatriationService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(NriRepatriationService));
            Assert.IsTrue(_service.GetType().Name == "NriRepatriationService");
        ...
        {
            var s1 = new NriRepatriationService();
            var s2 = new NriRepatriationService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new NriRepatriationService()).ToList();
        ...
        {
            var type = typeof(NriRepatriationService);
            Assert.IsNotNull(type);
            Assert.AreEqual("NriRepatriationService", type.Name);
        ...
        {
            var ns = typeof(NriRepatriationService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(NriRepatriationService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new NriRepatriationService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(NriRepatriationService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("NriRepatriationService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new NriRepatriationService();
