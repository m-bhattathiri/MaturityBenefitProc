...
{
    [TestClass]
    public class FundSwitchingServiceIntegrationTests
    {
        private FundSwitchingService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(FundSwitchingService));
            Assert.IsTrue(_service.GetType().Name == "FundSwitchingService");
        ...
        {
            var s1 = new FundSwitchingService();
            var s2 = new FundSwitchingService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new FundSwitchingService()).ToList();
        ...
        {
            var type = typeof(FundSwitchingService);
            Assert.IsNotNull(type);
            Assert.AreEqual("FundSwitchingService", type.Name);
        ...
        {
            var ns = typeof(FundSwitchingService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(FundSwitchingService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new FundSwitchingService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(FundSwitchingService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("FundSwitchingService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new FundSwitchingService();
