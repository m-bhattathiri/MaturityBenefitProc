...
{
    [TestClass]
    public class UnitCancellationServiceIntegrationTests
    {
        private UnitCancellationService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(UnitCancellationService));
            Assert.IsTrue(_service.GetType().Name == "UnitCancellationService");
        ...
        {
            var s1 = new UnitCancellationService();
            var s2 = new UnitCancellationService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new UnitCancellationService()).ToList();
        ...
        {
            var type = typeof(UnitCancellationService);
            Assert.IsNotNull(type);
            Assert.AreEqual("UnitCancellationService", type.Name);
        ...
        {
            var ns = typeof(UnitCancellationService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(UnitCancellationService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new UnitCancellationService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(UnitCancellationService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("UnitCancellationService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new UnitCancellationService();
