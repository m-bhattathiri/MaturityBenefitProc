...
{
    [TestClass]
    public class TreatyCalculationServiceIntegrationTests
    {
        private TreatyCalculationService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(TreatyCalculationService));
            Assert.IsTrue(_service.GetType().Name == "TreatyCalculationService");
        ...
        {
            var s1 = new TreatyCalculationService();
            var s2 = new TreatyCalculationService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new TreatyCalculationService()).ToList();
        ...
        {
            var type = typeof(TreatyCalculationService);
            Assert.IsNotNull(type);
            Assert.AreEqual("TreatyCalculationService", type.Name);
        ...
        {
            var ns = typeof(TreatyCalculationService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(TreatyCalculationService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new TreatyCalculationService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(TreatyCalculationService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("TreatyCalculationService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new TreatyCalculationService();
