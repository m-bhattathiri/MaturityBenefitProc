...
{
    [TestClass]
    public class PoolAllocationServiceIntegrationTests
    {
        private PoolAllocationService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(PoolAllocationService));
            Assert.IsTrue(_service.GetType().Name == "PoolAllocationService");
        ...
        {
            var s1 = new PoolAllocationService();
            var s2 = new PoolAllocationService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new PoolAllocationService()).ToList();
        ...
        {
            var type = typeof(PoolAllocationService);
            Assert.IsNotNull(type);
            Assert.AreEqual("PoolAllocationService", type.Name);
        ...
        {
            var ns = typeof(PoolAllocationService).Namespace;
            Assert.IsNotNull(ns);
        ...
        {
            var interfaces = typeof(PoolAllocationService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new PoolAllocationService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(PoolAllocationService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("PoolAllocationService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new PoolAllocationService();
