...
{
    [TestClass]
    public class BeneficiaryShareAllocationServiceIntegrationTests
    {
        private BeneficiaryShareAllocationService _service;
        ...
        public void Integration_ServiceInstantiation_Succeeds()
        {
            ...
            Assert.IsInstanceOfType(_service, typeof(BeneficiaryShareAllocationService));
            Assert.IsTrue(_service.GetType().Name == "BeneficiaryShareAllocationService");
        ...
        {
            var s1 = new BeneficiaryShareAllocationService();
            var s2 = new BeneficiaryShareAllocationService();
            Assert.IsNotNull(s1);
            Assert.IsNotNull(s2);
        ...
        {
            var instances = Enumerable.Range(0, 10).Select(_ => new BeneficiaryShareAllocationService()).ToList();
        ...
        {
            var type = typeof(BeneficiaryShareAllocationService);
            Assert.IsNotNull(type);
            Assert.AreEqual("BeneficiaryShareAllocationService", type.Name);
        ...
        {
            var ns = typeof(BeneficiaryShareAllocationService).Namespace;
            Assert.IsNotNull(ns);
            Assert.IsTrue(ns.Contains("MaturityBenefitProc"));
            Assert.IsTrue(ns.Contains("BeneficiaryAndLegalHeirs"));
        ...
        {
            var interfaces = typeof(BeneficiaryShareAllocationService).GetInterfaces();
        ...
        public void Integration_Constructor_NoException()
        {
            ...
            try { var svc = new BeneficiaryShareAllocationService(); }
            catch (Exception ex) { caught = ex; }
            Assert.IsNull(caught);
            Assert.IsNotNull(_service);
            Assert.IsTrue(true);
            Assert.IsFalse(false);
            Assert.AreEqual(typeof(BeneficiaryShareAllocationService), _service.GetType());
        }
        ...
        public void Integration_ToString_NotNull()
        {
            ...
            Assert.IsTrue(str.Contains("BeneficiaryShareAllocationService") || str.Contains("MaturityBenefitProc"));
        ...
        {
            var other = new BeneficiaryShareAllocationService();
